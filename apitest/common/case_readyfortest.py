...
def data_form(product_id, parameter_index, body_index, case_list, variable_list):
    """
    转换 body 和 parameter 的格式成 json 格式，参数的值如果有可选范围，则按照所给范围生成 N 条case
    :param product_id: 项目 ID
    :param parameter_index: parameter 在 1 条用例中的序号
    :param body_index: body 在 1 条用例中的序号
    ...
    for case in case_list:
        ...
        if case[body_index] != {}:  # 处理body的
            enum_data(case[body_index], n, case, new_case_list, body_index, product_id, case_list, variable_list)
        elif case[parameter_index] != {}:
            # 处理parameters的
            enum_data(case[parameter_index], n, case, new_case_list, parameter_index, product_id, case_list,
                      variable_list)
        else:
            # 如果body和param都是空的，那就直接变成case
            new_case_list.append(case_list[n])
        n += 1
    return new_case_list  # 这个list是最终的case


def enum_data(case, case_index, case_full_info, new_case_list, param_index, product_id, case_list, variable_list):
    # 这个方法是处理接口参数里的enum
    ...
    if not enum_count:  # 参数里面没有enum类型的时候，只要正常替换参数就好
        ...
        #
        #         else:
        #             value = data_replace(param, case[param], product_id, case_full_info[1], variable_list)  # 二级json的逻辑都在data_replace里处理（不包含耳机里面还有enum的情况）
        #         case[param] = value
        # else:
        #     for param in case:
        #         # print('there')
        ...
        for param in case:
            value = data_replace(param, case[param], product_id, case_full_info[1],
                                 variable_list)  # 二级json的逻辑都在data_replace里处理（不包含二级里面还有 enum 的情况）
            case[param] = value
        new_case_list.append(copy.deepcopy(case_list[case_index]))
        ...
        for i in range(m):
            ...
            for param in case:
                if param in enum_dict:
                    ...
                    new_case_m.update({param: data_replace(param, case[param], product_id, case_full_info[1], variable_list)})
            if param_index == 4:
            ...
            if 'enum' in case[param]:
                ...
                for enum_value in enum:
                    ...
                    if case[param]['type'] == 'String[]':
                        new_case_list[-1][param_index][param] = [enum_value]
                    else:
                        new_case_list[-1][param_index][param] = enum_value
        for item in new_case_list[-how_many:]:  # 这里终于对了
            for param in item[param_index]:
                if 'type' in item[param_index][param]:
                    value = data_replace(param, item[param_index][param], 2, case_full_info[1], variable_list)
                    item[param_index][param] = value
